...
import 'finding_controller.dart';

class FindingScreen extends GetView<FindingController> {
  FindingScreen({Key? key}) : super(key: key);
  TextEditingController name = TextEditingController();
  @override
  Widget build(BuildContext context) {
   
    controller.fetchAllFinding();
    return CustomerColumn(
      onPressed: () =>
          _showMyDialog(context, 'Yeni tapıntı əlavə et', 'Əlavə et', 1),
      title: AppConstantsText.finding,
      children: [
        CustomerSearchTextField(
          onChanged: (value) {
            controller.findingSearch(text: value);
          },
        ),
        const Divider(),
        GetBuilder<FindingController>(builder: (controller) {
          int a = 1;
                            ...
                            cells: [
                              ...
                              DataCell(Row(
                                children: [
                                  IconButton(
                                    onPressed: () {
                                      controller.findingVisible('1', e.id);
                                    },
                                    icon: const Icon(
                                      Icons.delete,
                                      color: Colors.red,
  ...
  List<Widget> buildDialogButtons(
    ...
    return <Widget>[
      TextButton(
        ...
        onPressed: () {
          if (count == 1) {
            controller.findingAdd(name.text);
          } else {
            controller.findingUpdate(name.text, id);
          }
