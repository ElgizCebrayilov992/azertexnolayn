...
class PositionScreen extends GetView<PositionController> {
  ...
  Widget build(BuildContext context) {
    ...
    controller.fetchAllPosition();
    return CustomerColumn(
      onPressed: () {
        name = TextEditingController(text: '');
          _showMyDialog(context, 'Yeni adiyyat əlavə et', 'Əlavə et', 1);},
      title: AppConstantsText.position,
      children: [
        CustomerSearchTextField(
          onChanged: (value) {
            controller.searchPosition(text: value);
          },
        ),
        const Divider(),
        GetBuilder<PositionController>(builder: (controller) {
          int a = 1;
                            ...
                            cells: [
                              ...
                              DataCell(Row(
                                children: [
                                  IconButton(
                                    onPressed: () {
                                      controller.visiblePosition('1', e.id);
                                    },
                                    icon: const Icon(
                                      Icons.delete,
                                      color: Colors.red,
  ...
  List<Widget> buildDialogButtons(
    ...
    return <Widget>[
      TextButton(
        ...
        onPressed: () {
          if (count == 1) {
            controller.addPosition(name.text);
          } else {
            controller.updatePosition(name.text, id);
          }
